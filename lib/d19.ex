...
defmodule Mining do
  ...
  def parse(lines) do
    ...
    |> Enum.map(fn line ->
      ...
      r =
        Regex.run(
          ~r/Blueprint ([0-9]+).*([0-9]+).* ([0-9]+).* ([0-9]+).* ([0-9]+).* ([0-9]+).* ([0-9]+).*/,
          line,
          capture: :all_but_first
        )

      [id, o_ore, c_ore, ob_ore, ob_clay, g_ore, g_ob] = r |> Enum.map(&String.to_integer/1)
  ...
  def attempt_skipping_spawn(bp, amts, rates, rem_time, spawn_target) do
    ...
    if possible? == true do
      ...
      ticks_needed =
        case spawn_target do
          :ore ->
            ceil((bp.o_ore - amts.ore) / rates.ore)

          :clay ->
            ceil((bp.c_ore - amts.ore) / rates.ore)
  ...
  def calc_geode_upper_bound(%Amounts{} = amts, %Rates{} = rates, bp, rem_time) do
    ...
    {_, _, ore_max_seq, obsidian_max_seq} =
      Enum.reduce(rem_time..1//-1, {amts, rates, [amts.ore], [amts.obsidian]}, fn _,
                                                                                  {amt_acc,
                                                                                   rate_acc,
                                                                                   ore_max_seq_acc,
                                                                                   obsidian_max_seq_acc} ->
        # NOTE: we are ignoring bot costs in these bounds
        # try to spawn ore bot

        # try to spawn all bots besides geode bot + update rates
        n_r =
          Enum.reduce([:ore, :clay, :obsidian], rate_acc, fn bot, n_rate_acc ->
    ...
    {geode_upper_bound, _, _, _} =
      Enum.reduce(zipped, {amts.geode, rates.geode, 0, 0}, fn {cur_ore, cur_obsidian},
                                                              {g, g_rate, decr_ore, decr_obsidian} ->
  ...
  def simulate(bp, amts, rates, rem_time, max_seen) do
    ...
    if geode_upper_bound > max_seen do
      ...
      |> Enum.filter(fn bot ->
        case bot do
          :ore ->
            rates.ore < bp.o_ore or rates.ore < bp.c_ore or rates.ore < bp.ob_ore or
              rates.ore < bp.g_ore

          :clay ->
            rates.clay < bp.ob_clay

          :obsidian ->
            rates.obsidian < bp.g_ob
