defmodule PacketParser do
  def validate_next(window, packet_size, buffer, pos_idx)
      when length(window) == packet_size and length(buffer) > 0 do
...
defmodule D6 do
  def sol(input) do
    buffer = input |> String.to_charlist()

    args =
      [4, 14]
      |> Enum.map(&{Enum.take(buffer, &1), &1, Enum.drop(buffer, &1), &1})

    [start_packet_idx_p1, start_packet_idx_p2] =
      Enum.map(
        args,
        &PacketParser.validate_next(elem(&1, 0), elem(&1, 1), elem(&1, 2), elem(&1, 3))
      )
