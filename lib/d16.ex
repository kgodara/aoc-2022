...
defmodule ValveNetwork do
    ...
    valve_lookup =
      ...
      |> Enum.map(fn line ->
        Regex.named_captures(
          ~r/(?<label>[A-Z][A-Z]).*flow rate=(?<flow>[0-9]+).*valves? (?<adj>.*)/,
          line
        )
      end)
      |> Enum.reduce(%{}, fn captures, lookup ->
        ...
        Map.put(lookup, map_size(lookup), %Valve{
          id: map_size(lookup),
          label: label,
          flow: flow,
          adj_labels: neighbours
        })
      end)
  ...
  def permute_pruned(_, _, closed_valves, score, rem_time, limits)
      when rem_time == 0 or closed_valves == [] do
    {score, limits}
  end
