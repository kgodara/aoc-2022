...
defmodule KeepAway do
    ...
    {cur_monkey, monkey_lookup} =
      cond do
        String.contains?(cur_line, "Monkey ") ->
          monkey_num =
            Regex.run(~r/[[:digit:]]/, cur_line, capture: :first)
            |> List.first()
            |> String.to_integer()
  ...
  def items_to_mod_list(monkey_lookup) do
    # get all div by vals
    div_by_map =
      Map.values(monkey_lookup)
      |> Enum.map(& &1.test)
      |> Enum.reduce(%{}, fn v, m -> Map.put(m, v, nil) end)
    ...
    if Map.has_key?(monkey_lookup, cur_monkey) do
      ...
      monkey_lookup =
        ...
        |> Enum.reduce(monkey_lookup, fn item, map ->
          ...
          |> Map.update!(
            if(Kernel.rem(worry, m.test) == 0, do: m.pass, else: m.fail),
            &%{&1 | items: [worry] ++ &1.items}
          )
        end)
    ...
    if Map.has_key?(monkey_lookup, cur_monkey) do
      ...
      monkey_lookup =
        ...
        |> Enum.reduce(monkey_lookup, fn item, map ->
          ...
          # add updated item to pass/fail monkey
          |> Map.update!(
            if(Map.get(new_item, m.test) == 0, do: m.pass, else: m.fail),
            &%{&1 | items: [new_item] ++ &1.items}
          )
        end)
