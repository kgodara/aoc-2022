...
defmodule SpaceManager do
    ...
    n_entry_lookup =
      cond do
        ...
        String.match?(cur_cmd, ~r/[0-9]+ .*/) ->
          ...
          |> Map.update!(cwd_path, fn entry ->
            %{entry | children: [full_file_name] ++ entry.children}
          end)
        ...
        String.match?(cur_cmd, ~r/dir .*/) ->
          ...
          |> Map.update!(cwd_path, fn entry ->
            %{entry | children: [full_dir_name] ++ entry.children}
          end)
  ...
  def fill_dir_sizes(entry_lookup) do
    ...
    dir_by_depth =
      ...
      |> Enum.map(fn e ->
        {e, String.graphemes(e.name) |> Enum.count(&(&1 == "/"))}
      end)
...
defmodule D7 do
  def sol(input) do
    ...
    entry_lookup =
      SpaceManager.parse_layout(lines, [], %{
        "/" => %Entry{name: "/", type: :dir, size: 0, children: []}
      })
      |> SpaceManager.fill_dir_sizes()
