...
defmodule HillClimb do
  ...
  def parse_grid(lines) do
    ...
    # start = 'S'
    # end = 'E'
    {start_coord, end_coord} = Enum.reduce(grid, {%Coord{}, %Coord{}}, fn {{row_idx, col_idx}, val}, {start_c, end_c} ->
      {
  ...
  # end found
  def traverse_grid([{pos, steps_taken} | _rest], _grid, {_num_rows, _num_cols}, _seen_coords, end_pos) when pos == end_pos do
    steps_taken
  end
  ...
  def traverse_grid([{src, steps_taken} | dst_q], grid, {num_rows, num_cols}, seen_coords, end_pos) do
    ...
    valid_dst_list = candidates |>
      Enum.filter(& (&1.row >= 0 and &1.row < num_rows) and (&1.col >= 0 and &1.col < num_cols) and (&1 not in seen_coords)) |>
    ...
    new_seen_coords = valid_dst_list |>
      Enum.reduce(seen_coords, fn dst, seen -> seen |> MapSet.put(dst) end)

    HillClimb.traverse_grid(dst_q ++ dst_to_add, grid, {num_rows, num_cols}, new_seen_coords, end_pos)
  end
end
...
min_path_default_start = HillClimb.traverse_grid([{start_c, 0}], grid, {num_rows, num_cols}, MapSet.new([start_c]), end_c)

min_path_all_starts = possible_starts |>
  Enum.map(& HillClimb.traverse_grid([{&1, 0}], grid, {num_rows, num_cols}, MapSet.new([&1]), end_c)) |>
  Enum.filter(& &1 != nil) |>
  Enum.min
