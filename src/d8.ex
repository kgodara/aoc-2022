...
defmodule Tree do
  ...
  def max_scenic_score(grid, {row_idx, col_idx}, num_rows, max_score) do
    ...
    above = & Enum.reduce((row_idx-1)..0//-1, [], fn i, acc -> List.insert_at(acc, 0, Enum.at(&1[i], col_idx)) end ) |>
          Enum.reverse
    below = & Enum.reduce((row_idx+1)..(num_rows-1), [], fn i, acc -> List.insert_at(acc, 0, Enum.at(&1[i], col_idx)) end ) |>
          Enum.reverse

    check_with_bounds = fn cond, {to_edge, num_to_edge} -> if cond == true, do: 0, else: find_1_based_index.(to_edge.(grid), num_to_edge) end
...
right_down_grid = grid |> Tree.mirror_grid(row_num) |> Tree.visible_left_top |> Tree.mirror_grid(row_num)

# get number of visible trees, checking mirrored and unmirrored grids (|||)
res = Enum.zip(left_up_grid, right_down_grid) |>
  Enum.reduce(0,
    fn {{_, v1}, {_, v2}}, acc ->
