...
defmodule MonkeyMath do
    ...
    |> Enum.map(fn line ->
      ...
      case length(parts) do
        2 ->
          ...
          %MonkeyOp{
            label: label |> String.trim_trailing(":"),
            type: :literal,
            res: lit |> String.to_integer()
          }
  ...
  def exec(unresolved, resolved, unresolvable) do
    ...
    if start_unresolved != length(unresolved) do
      MonkeyMath.exec(unresolved, resolved, unresolvable)
    else
      resolved
    end
  end
  ...
  def resolve_from_root(cur_op, {ops, resolved}, target_val) do
    ...
    {n_l_val, n_r_val} =
      case {l_val, r_val} do
        ...
        {nil, nil} ->
          raise "Both children are undefined"
      end
    ...
    MonkeyMath.resolve_from_root(n_op, {ops, resolved}, n_target_val)
  end
end

defmodule D21 do
  def sol(input) do
    ops = MonkeyMath.parse(input)

    p1_res = MonkeyMath.exec(ops)
    ...
    resolved = MonkeyMath.exec(ops_filtered, %{}, MapSet.new(["humn"]))
    ...
    p2_res =
      MonkeyMath.resolve_from_root(Map.get(ops_map, start_label), {ops_map, resolved}, target_val)
