...
defmodule Board do
  ...
  def adj_empty_at_tick(board, {r, c}, start, ticks_to_simulate) do
    ...
    Enum.filter(fn {n_r, n_c} ->
      ...
      ({n_r, n_c} == start and {r,c} == start) or (n_r >= 0 and n_c >= 0 and n_r <= board.down_bound and n_c <= board.right_bound)
    end) |>
    Enum.filter(fn {n_r, n_c} ->
      ...
      [right_blizz?, down_blizz?, left_blizz?, up_blizz?] == [false, false, false, false]
      or
    ...
    Enum.reduce_while(valid_moves, {min_steps, seen}, fn move, {min_steps_acc, seen_acc} ->
      ...
      if (steps_taken+1) >= min_steps_acc do
        ...
        case MapSet.member?(seen_acc, {elem(move,0), elem(move,1), steps_taken + 1}) do
          ...
          false ->
            {n_min, n_seen} = BlizzardBasin.traverse(board,
              {start, dst},
...
defmodule Main do
  def main do
    ...
    fn {start, dst}, res_list ->
      steps = BlizzardBasin.traverse(board, {start, dst}, {start, res_list |> hd(), :infinity}) |> elem(0)
      [steps] ++ res_list
    end) |>
    Enum.reverse
