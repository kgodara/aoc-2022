...
defmodule Parse do
      ...
      |> Enum.reduce({%{}, %{}}, fn {line, line_idx}, {board_map, bounds_map} ->
        ...
        bounds_map =
          ...
          |> Enum.reduce(bounds_map, fn {{line_idx, ch_idx}, _}, bounds_acc ->
            # top bound
            bounds_acc =
              case Map.get(bounds_acc, {:top, ch_idx}) do
                nil ->
  ...
  def parse_cube_faces(input, cube_line_height) do
    ...
    {_, cube_face_map} =
      Enum.reduce(1..6, {flattened_lines, %{}}, fn cube_face, {normalized_lines, face_lookup} ->
        {face_chars, rem} =
          normalized_lines
...
defmodule MonkeyMap do
  ...
  def traverse([c_dir | rem_dir], map, cursor) do
    n_cursor =
      Enum.reduce_while(0..(c_dir - 1), cursor, fn _step, cursor_acc ->
...
defmodule D22 do
  def sol(input) do
    ...
    cursor_p1 =
      MonkeyMap.traverse(directions, %MapFlat{grid: board, bounds: bounds}, %Cursor{
        row: r,
        col: c,
        orientation: :right
      })
    ...
    # Part 2:
    cube_side_len = if input |> String.split("\n") |> length < 20, do: 4, else: 50
    ...
    cursor_p2 =
      MonkeyMap.traverse(
        directions,
        %MapCube{face_lookup: cube_face_map, side_len: cube_side_len},
        %Cursor{row: 0, col: 0, orientation: :right, face: 1}
      )
