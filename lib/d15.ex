...
defmodule BeaconExclusion do
  def parse(lines) do
    ...
    |> Enum.map(
      &Regex.named_captures(
        ~r/.*x=(?<s_x>-?[0-9]+), y=(?<s_y>-?[0-9]+).*x=(?<b_x>-?[0-9]+), y=(?<b_y>-?[0-9]+)/,
        &1
      )
    )
  ...
  def init_pivot_range(row_y, sensor) do
    ...
    %PivotRange{
      range: (s_x - remaining_steps)..(s_x + remaining_steps),
      pivot: elem(sensor.sensor, 1)
    }
  end
  ...
  def sort_pivot_ranges(ranges) do
    ranges
    |> Enum.sort(
      &(&1.range.first < &2.range.first or
          (&1.range.first == &2.range.first and &1.range.last < &2.range.last))
    )
  end
    ...
    {rem, merged} =
      cond do
        ...
        cur_r.last >= next_r.first ->
          {[
             %PivotRange{pivot: nil, range: cur_r.first..max(cur_r.last, next_r.last)} | trunc_rem
           ], merged}
...
defmodule D15 do
  def sol(input) do
    ...
    # if String.contains?(file_str, "d15_ex"), do: 20, else: 2_000_000
    exclude_row = if length(lines) < 20, do: 20, else: 2_000_000

    lower_bound = 0
    # if String.contains?(file_str, "d15_ex"), do: 20, else: 4_000_000
    upper_bound = if length(lines) < 20, do: 20, else: 4_000_000
    ...
    # Part 2
    p2_res =
      Enum.reduce(lower_bound..upper_bound, {[], sensors, nil}, fn cur_row,
                                                                   {prev_active, prev_inactive,
                                                                    res} ->
        if res != nil do
          {nil, nil, res}
        else
    ...
    p2_res = p2_res_col * 4_000_000 + p2_res_row
