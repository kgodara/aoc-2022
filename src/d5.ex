defmodule Stacks do
  ...
  def populate_stacks(lines, empty_stacks, stack_count) do
    ...
    Enum.map(& " " <> &1 ) |>
    Enum.reduce(empty_stacks, fn line, accum -> Stacks.update_stacks_from_line(line, accum, 0) end)
  end
...
stack_count =  " " <> Enum.at(lines, 0) |>
  String.length |>
  div(4)

# init stacks based on number of columns
empty_stacks = Enum.map(0..(stack_count-1), &{&1, []}) |>
  Map.new

# parse and populate stacks then
# reverse stacks so top element is at index 0
pop_stacks = Stacks.populate_stacks(lines, empty_stacks, stack_count) |>
...
moved_p1 = Enum.reduce(lines, pop_stacks, fn line, accum -> Stacks.exec_move_p1(line, accum) end)
moved_p2 = Enum.reduce(lines, pop_stacks, fn line, accum -> Stacks.exec_move_p2(line, accum) end)
