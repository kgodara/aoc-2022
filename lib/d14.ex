...
defmodule Cave do
      ...
      |> Enum.map(fn rock_struct ->
        ...
        struct_coords =
          ...
          |> Enum.map(fn path_segment ->
            ...
            Enum.reduce(if(is_horizontal, do: y1..y2, else: x1..x2), [], fn point_idx,
                                                                            coord_list ->
  ...
  def get_next_impacted_node_id(_, _, upper_bound, lowest_node_height)
      when upper_bound > lowest_node_height do
    :abyss
  end
  ...
  def get_next_impacted_node_id(coord_id_lookup, x, upper_bound, lowest_node_height) do
    case Map.get(coord_id_lookup, {x, upper_bound}) do
      impacted_node_id when not is_nil(impacted_node_id) ->
        impacted_node_id

      nil ->
        Cave.get_next_impacted_node_id(coord_id_lookup, x, upper_bound + 1, lowest_node_height)
    end
  end
  ...
  def drop_sand(coord_id_lookup, id_node_lookup, x, upper_bound, lowest_node_height) do
    impacted_node =
      id_node_lookup
      |> Map.get(
        Cave.get_next_impacted_node_id(coord_id_lookup, x, upper_bound, lowest_node_height)
      )
    ...
    case impacted_node do
      ...
      impacted_node ->
        left_impact =
          case Map.has_key?(coord_id_lookup, {x - 1, impacted_node.y}) do
            false ->
              Cave.drop_sand(
                coord_id_lookup,
                id_node_lookup,
  ...
  def add_floor_nodes(coord_id_lookup, id_node_lookup, lowest_node_height) do
    ...
    {coord_id_lookup, id_node_lookup} =
      Cave.add_node(coord_id_lookup, id_node_lookup, {500, floor_y})
  ...
  def drop_until_abyss(
    ...
    case Cave.drop_sand(coord_id_lookup, id_node_lookup, x, upper_bound, lowest_node_height) do
      ...
      dropped_sand when dropped_sand.x == 500 and dropped_sand.y == 0 ->
        num_iter + 1

      dropped_sand ->
        {coord_id_lookup, id_node_lookup} =
          Cave.add_node(coord_id_lookup, id_node_lookup, {dropped_sand.x, dropped_sand.y})
...
defmodule D14 do
  def sol(input) do
    ...
    IO.inspect(
      Cave.drop_until_abyss(coord_id_lookup, id_node_lookup, 500, 0, lowest_node_height, 0),
      label: "Part 1"
    )

    {coord_id_lookup, id_node_lookup} =
      Cave.add_floor_nodes(coord_id_lookup, id_node_lookup, lowest_node_height)
